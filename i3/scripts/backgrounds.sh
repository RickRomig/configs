...
# Description  : Set wallpaper directory in i3 X-session
# Dependencies : feh
# Arguments    : None
...
select_background_directory() {
	local -r local_host="${HOSTNAME:-$(hostname)}"
	local -r bg_dirs=("$HOME/Pictures/wallpaper" "$HOME/Pictures/backgrounds" "$HOME/.config/backgrounds")
	local bg_dir
	case "$local_host" in
		hp-850-g3 )
